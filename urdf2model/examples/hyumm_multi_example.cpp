...
int main()
{
  ...
  // ---------------------------------------------------------------------
  std::string urdf_filename = ws_path+"/urdf2model/models/HYUMM/hyumm_multi.urdf";
  ...
  std::vector<std::string> required_Frames = {"base_center", "l_joint0", "l_joint1", "l_joint2", "l_joint3", "l_joint4", "l_joint5", "l_indy7_tcp",
                                              "r_joint0", "r_joint1", "r_joint2", "r_joint3", "r_joint4", "r_joint5", "r_indy7_tcp"};

  std::string base_name = "base_center";
  std::string l_end_effector_name = "l_indy7_tcp";

  casadi::Function fkpos_ee = robot_model.forward_kinematics("position", l_end_effector_name);
  casadi::Function fkrot_ee = robot_model.forward_kinematics("rotation", l_end_effector_name);
  casadi::Function fk_ee = robot_model.forward_kinematics("transformation", l_end_effector_name);
  ...
  casadi::Function J_s = robot_model.kinematic_jacobian("space", l_end_effector_name);
  casadi::Function J_b = robot_model.kinematic_jacobian("body", l_end_effector_name);
  ...
  mecali::generate_code(fd, "hyumm_multi_fd", codegen_options);
  mecali::generate_code(CoM_x, "hyumm_multi_CoM_x", codegen_options);
  mecali::generate_code(J_com, "hyumm_multi_J_com", codegen_options);
  mecali::generate_code(id, "hyumm_multi_id", codegen_options);
  mecali::generate_code(M, "hyumm_multi_M", codegen_options);
  mecali::generate_code(Minv, "hyumm_multi_Minv", codegen_options);
  mecali::generate_code(C, "hyumm_multi_C", codegen_options);
  mecali::generate_code(G, "hyumm_multi_G", codegen_options);
  //mecali::generate_code(fk_ee_pos, "mmo500_ppr_fk_ee_pos", codegen_options);
  mecali::generate_code(fkrot_ee, "hyumm_multi_fkrot_ee", codegen_options);
  mecali::generate_code(fk_ee, "hyumm_multi_fk_ee", codegen_options);
  mecali::generate_code(fk_base, "hyumm_multi_fk_base", codegen_options);
  mecali::generate_code(fk, "hyumm_multi_fk", codegen_options);
  mecali::generate_code(J_fd, "hyumm_multi_J_fd", codegen_options);
  mecali::generate_code(J_id, "hyumm_multi_J_id", codegen_options);
  mecali::generate_code(J_s, "hyumm_multi_J_s", codegen_options);
  mecali::generate_code(J_b, "hyumm_multi_J_b", codegen_options);

  robot_model.generate_json("hyumm_multi.json");
